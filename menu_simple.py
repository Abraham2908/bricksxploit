"""
Simple menu system for BricksXploit
"""
...
def show_menu(config):
    ...
    while True:
        ...
        ui.update_screen(
            "Main Menu",
            menu_items,
            Text("Welcome to BricksXploit! Select an option from the menu.")
        )
        ...
        if choice == "1":
            ...
            ui.update_screen(
                "Main Menu",
                menu_items,
                Text("Exiting BricksXploit. Goodbye!", style="green")
            )
            time.sleep(1.0)
            sys.exit(0)
...
def validate_menu(config):
    ...
    ui.update_screen(
        "Validate Credentials",
        menu_items,
        Text("Validate Credentials: Test access to a Databricks workspace.")
    )
    ...
    if user_info:
        ...
        if user_info:
            ...
            ui.update_screen(
                "Validate Credentials",
                menu_items,
                Text(f"Successfully validated credentials for: {workspace}\nLogged in as: {user_display}", style="green")
            )
    else:
        ui.add_notification("Invalid credentials or workspace not accessible.", "ERROR")
        ui.update_screen(
            "Validate Credentials",
            menu_items,
            Text("Failed to validate credentials. Please try again.", style="red")
        )
        return
...
def config_menu(config):
    ...
    while True:
        ...
        ui.update_screen(
            "Configuration",
            menu_items,
            Text("Configuration: Manage application settings.")
        )
...
def discord_settings(config):
    ...
    ui.update_screen(
        "Configuration",
        menu_items,
        Text("Discord Settings: Configure Discord webhook for notifications.")
    )
